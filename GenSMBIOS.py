...
class Smbios:
    ...
    def _get_plist(self):
        ...
        if p.lower() == "q":
            ...
            self.plist_data = None
            return
        return self.load_plist(p)

    def load_plist(self, p):
    ...
    def _generate_smbios(self, macserial):
        ...
        if times > 20:
            times = 20
        self._create_smbios(macserial,smtype,times)

    def _create_smbios(self, macserial, smtype, times, interactive = True):
    ...
    def run(self, install = False, config_plist = None, smbios = None):
        self.u.head()
        print("")
        macserial = self._get_binary()
        if macserial:
        ...
        if install:
            self._get_macserial()
        if config_plist:
            self.load_plist(args.config_plist)
        if smbios:
            self._create_smbios(macserial, smbios, 1, False)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(prog="GenSMBIOS", description="Generate SMBIOS for OpenCore")
    parser.add_argument('-i', '--install', action='store_true', help='install macserial binary')
    parser.add_argument('-c', '--config-plist', type=str, help=f'config.plist file to update')
    parser.add_argument('-s', '--smbios', type=str, help=f'Please type the SMBIOS to gen i.e. iMac18,3')
    args = parser.parse_args()
    s = Smbios()

    if args.install or args.config_plist or args.smbios:
        s.run(args.install, args.config_plist, args.smbios)
        exit(0)
    while True:
        try:
            s.main()
